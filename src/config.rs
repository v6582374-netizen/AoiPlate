...
impl AppConfig {
    pub fn sanitize(&mut self) {
        ...
        if self.min_diameter_px < 80 || self.min_diameter_px > 260 {
            self.min_diameter_px = 120;
        }
        if self.max_diameter_px < 180 || self.max_diameter_px > 520 {
            self.max_diameter_px = 320;
        }
